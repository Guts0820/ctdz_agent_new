...
教师作业页提供“录入标准答案题目”入口，支持移动端后置摄像头拍照和本地 JPEG、PNG、WebP、BMP 图片。教师需选择适用年级，可选学期；前端校验 10 MB 大小限制后，以 `multipart/form-data` 调用 `/api/v1/teacher/question-imports/preview`。界面分别展示上传、OCR 和 LLM 复核状态，并在请求期间阻止重复提交。逐题编辑和裁决界面由后续 C2 阶段承接。

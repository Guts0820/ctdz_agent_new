...
| 答错后三级错因分析与知识点关联 | 已实现 |
| 疑似抄袭判断 | 已取消（不实现） |
| 苏格拉底式引导追问与多轮对话 | 已取消（不实现） |
| 知识点讲解、常见错误与教学提示检索 | 已实现 |
| 讲解、提示与变式题生成 | 已实现 |
| BASIC / STANDARD / ADVANCED 教学分档 | 已实现 |
| Mastery + Priority 联动更新 | 已实现 |

注：本表只做流程节点摘要。当前实现状态、缺口和验收标准以 `docs/项目完整工作流.md` 为准；各模块的权威说明见对应目录的 `AGENTS.md` 与 `docs/README.md`。`docs/backend/` 下的契约、状态机和 API 文档是最初的设计稿，不代表现状。

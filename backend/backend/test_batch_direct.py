...
def test_1_create_batch():
    """测试1：创建批次"""
    print_section("测试1：创建作业批次（3道题，默认locked）")

    batch_id = create_batch("C-001", "T-001", "2026-08-04", ["Q-0001", "Q-0002", "Q-0003"])
    print(f"[OK] 批次创建成功: {batch_id}")

    return batch_id

def test_2_check_locked(question_ids):
    """测试2：验证locked状态题目"""
    ...
    for qid in question_ids:
        ...
        if not released:
            print(f"  ✅ 验证通过：题目{qid}答案未发布")
        else:
            print(f"  ❌ 验证失败：题目{qid}答案应该未发布")

def test_3_release_batch(batch_id):
    """测试3：一键放行批次"""
...
def test_4_check_released(question_ids):
    """测试4：验证released状态题目"""
    ...
    for qid in question_ids:
        ...
        if released:
            print(f"  ✅ 验证通过：题目{qid}答案已发布")
        else:
            print(f"  ❌ 验证失败：题目{qid}答案应该已发布")

def test_5_create_batch2():
    """测试5：创建第二个批次（用于精细放行）"""
    print_section("测试5：创建第二个批次（用于精细放行测试）")

    batch_id = create_batch("C-002", "T-001", "2026-08-05", ["Q-0004", "Q-0005"])
    print(f"✅ 批次创建成功: {batch_id}")

    return batch_id

def test_6_release_partial(batch_id):
    """测试6：精细放行部分题目"""
    print_section(f"测试6：精细放行批次 {batch_id} 的部分题目（只放行Q-0004）")

    release_partial(batch_id, ["Q-0004"])
    print(f"✅ 已放行题目 Q-0004")

def test_7_check_partial():
    """测试7：验证partial状态题目"""
    ...
    if released:
        print(f"  ✅ 验证通过：已放行的题目可以看到答案")
    else:
        print(f"  ❌ 验证失败：已放行的题目应该能看到答案")
    ...
    if not released:
        print(f"  ✅ 验证通过：未放行的题目看不到答案")
    else:
        print(f"  ❌ 验证失败：未放行的题目不应该能看到答案")

def test_8_check_old_question():
    """测试8：验证不属于任何批次的旧题目"""
    ...
    if released:
        print(f"  ✅ 验证通过：不属于任何批次的题目默认不限制")
    else:
        print(f"  ❌ 验证失败：不属于任何批次的题目应该默认不限制")

def main():
    print("=" * 60)
    print("  批次管理功能验证测试（直接函数测试）")
    print("=" * 60)

    # 测试1：创建批次
    batch_id_1 = test_1_create_batch()

    # 测试2：验证locked状态
    test_2_check_locked(["Q-0001", "Q-0002", "Q-0003"])

    # 测试3：一键放行
    test_3_release_batch(batch_id_1)

    # 测试4：验证released状态
    test_4_check_released(["Q-0001", "Q-0002", "Q-0003"])

    # 测试5：创建第二个批次
    batch_id_2 = test_5_create_batch2()

    # 测试6：精细放行
    test_6_release_partial(batch_id_2)

    # 测试7：验证partial状态
    test_7_check_partial()

    # 测试8：验证旧题目
    test_8_check_old_question()

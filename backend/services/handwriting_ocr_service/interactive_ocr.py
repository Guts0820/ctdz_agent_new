...
def run_interactive(
    ...
    while True:
        ...
        try:
            ...
            analysis_input = _read_validated_analysis_input(output_path)
        except Exception as error:
            print_fn(f"识别失败：{error}")
            continue

        print_fn(f"识别完成：{output_path}")
        print_fn(json.dumps(analysis_input, ensure_ascii=False, indent=2))
...
def _read_validated_analysis_input(output_path: Path) -> dict[str, object]:
    """Read the schema-validated judging input saved with an OCR result."""
    response = json.loads(output_path.with_suffix(".json").read_text(encoding="utf-8"))
    analysis_input = response.get("analysis_input")
    if not isinstance(analysis_input, dict):
        raise RuntimeError("OCR 未返回通过 JSON Schema 校验的 analysis_input。")

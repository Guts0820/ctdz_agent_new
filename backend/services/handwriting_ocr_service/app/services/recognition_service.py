...
class RecognitionService:
    ...
    def recognize(self, image_bytes: bytes, content_type: str, mode: str = "student_work") -> RecognitionResult:
        primary_result = self._recognize_engine(image_bytes, content_type, mode, self.primary_engine)
        ...
        if primary_requires_review and self.fallback_engine:
            try:
                result = self._recognize_engine(image_bytes, content_type, mode, self.fallback_engine)
                fallback_used = True
            except Exception:
    ...
    @staticmethod
    def _recognize_engine(image_bytes: bytes, content_type: str, mode: str, engine: RecognitionEngine) -> EngineResult:
        if mode == "standard_answer":
            return engine.recognize(image_bytes, content_type, mode=mode)  # type: ignore[call-arg]
        return engine.recognize(image_bytes, content_type)

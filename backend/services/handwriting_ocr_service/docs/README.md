...
网关应使用该对象的题干、作答、说明和视觉语义；不得消费调试字段 `raw_json`。当 `review_required=true` 或状态为 `low_confidence` 时，应先让用户确认识别内容。当前系统由网关以 `0.95` 为质量门槛：本服务仍会返回实际置信度，网关负责阻止低于该值的结果进入判题，并把通过校验的题干交给知识图谱解析标准答案。OCR 服务不判断答案正误。

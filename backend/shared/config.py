...
# JWT 签发密钥与有效期；未配置时启动会打印显式警告并使用进程内临时密钥
AUTH_JWT_SECRET = get_env("AUTH_JWT_SECRET", "")
AUTH_JWT_TTL_HOURS = get_int("AUTH_JWT_TTL_HOURS", 12)
...
TEACHER_SERVICE_URL = get_env("TEACHER_SERVICE_URL", "http://127.0.0.1:8090")
...
OCR_MIN_CONFIDENCE = get_float("OCR_MIN_CONFIDENCE", 0.80)
STANDARD_ANSWER_OCR_MIN_CONFIDENCE = get_float("STANDARD_ANSWER_OCR_MIN_CONFIDENCE", 0.95)
LLM_API_KEY = get_env("LLM_API_KEY", "")
LLM_BASE_URL = get_env("LLM_BASE_URL", "")
LLM_MODEL = get_env("LLM_MODEL", "")
QWEN_EMBEDDING_API_KEY = get_env("QWEN_EMBEDDING_API_KEY", get_env("QWEN_API_KEY", ""))
QWEN_EMBEDDING_BASE_URL = get_env(
    "QWEN_EMBEDDING_BASE_URL",
    get_env("QWEN_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1"),
)
QWEN_EMBEDDING_MODEL = get_env("QWEN_EMBEDDING_MODEL", "text-embedding-v3")
QWEN_EMBEDDING_DIMENSIONS = get_int("QWEN_EMBEDDING_DIMENSIONS", 1024)
QWEN_EMBEDDING_TIMEOUT_SECONDS = get_float("QWEN_EMBEDDING_TIMEOUT_SECONDS", 30.0)
KG_VECTOR_INDEX_NAME = get_env("KG_VECTOR_INDEX_NAME", "question_embedding_index")
KG_VECTOR_TOP_K = get_int("KG_VECTOR_TOP_K", 10)
